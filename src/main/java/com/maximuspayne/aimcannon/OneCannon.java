...
public class OneCannon {
	...
	boolean ignite;
	int cannonLength;
	public int cannonType; // 0 single, 1 double, 2 fireball (crap), 3 torpedo mk2, 4 depth charge, 5 depth
							// charger II, 6 triple cannon, 7 torp mk3, 8 torp mk1, 9 bombs
	int delay;
	int range;
	int torpedoMode;
	int depth;
	...
	public boolean isThisCannon(Location inloc, boolean skipValidation) {
		if ((inloc.getBlockX() == loc.getBlockX()) && (inloc.getBlockY() == loc.getBlockY()) && (inloc.getBlockZ() == loc.getBlockZ())) {
	...
	public boolean Charge(Player p, boolean leftClick) {
		...
		if ((inventory.getItem(4) == null) || (inventory.getItem(4).getTypeId() != 388)) {
			...
			if (PermissionInterface.CheckEnabledWorld(p.getLocation())) {
				if (NavyCraft.econ.has(p, cost) /* ess.getUser(p).canAfford(new BigDecimal(cost)) */) {
					p.sendMessage("Weapon purchased.");

					inventory.setItem(4, new ItemStack(388, 1));
					NavyCraft.econ.withdrawPlayer(p, cost);// getUser(p).takeMoney(new BigDecimal(cost));
				} else {
	...
	public void fireTorpedoMk2(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth, final int delayShoot, final boolean left) {
		// final int taskNum;
		// int taskNum =
		// plugin.getServer().getScheduler().scheduleAsyncDelayedTask(plugin, new
		// Runnable(){
		final Craft testCraft = Craft.getCraft(loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
		final Weapon torp = new Weapon(b, torpHeading, torpDepth);
		AimCannon.weapons.add(torp);
	...
	public void fireTorpedoMk3(final Player p, final Block b, final BlockFace torpHeading, final int torpDepth, final int delayShoot, final boolean left) {
		...
		if ((testCraft != null) && (torp.tubeNum != 0) && testCraft.tubeFiringMode.containsKey(torp.tubeNum)) {
			...
			// if( testCraft.tubeFiringMode.get(torp.tubeNum) == -1 && (onScopePlayer !=
			// null || testCraft.lastPeriscopeYaw != -9999) )
			// {
			/*
			 * float rotation=0; if( onScopePlayer != null) rotation = (float) Math.PI *
			 * onScopePlayer.getLocation().getYaw() / 180f; else rotation = (float) Math.PI
			 * * testCraft.lastPeriscopeYaw / 180f;
			 */
			float rotation = 0;
			int torpRotation = 0;
			if (direction == BlockFace.SOUTH) {
	...
	public boolean isValidCannon(Block b) {
		...
		if (b.getRelative(BlockFace.NORTH, 1).getType() == Material.DIAMOND_BLOCK) {
			...
			if ((b.getRelative(BlockFace.NORTH, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.NORTH, 2).getType() == Material.PUMPKIN)) {
				direction = BlockFace.NORTH;
				cannonType = 0;
				if (ammunition == -1) {
					ammunition = 40;
			...
			if ((b.getRelative(BlockFace.EAST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.EAST, 2).getType() == Material.PUMPKIN)) {
				direction = BlockFace.EAST;
				cannonType = 0;
				if (ammunition == -1) {
					ammunition = 40;
			...
			if ((b.getRelative(BlockFace.SOUTH, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.SOUTH, 2).getType() == Material.PUMPKIN)) {
				direction = BlockFace.SOUTH;
				cannonType = 0;
				if (ammunition == -1) {
					ammunition = 40;
			...
			if ((b.getRelative(BlockFace.WEST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.WEST, 2).getType() == Material.PUMPKIN)) {
				direction = BlockFace.WEST;
				cannonType = 0;
				if (ammunition == -1) {
					ammunition = 40;
			...
			if ((b.getRelative(BlockFace.NORTH, 1).getType() == Material.GOLD_BLOCK) && (b.getRelative(BlockFace.SOUTH, 1).getType() == Material.GOLD_BLOCK)) {
				if ((b.getRelative(BlockFace.NORTH_WEST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.SOUTH_WEST, 1).getType() == Material.PUMPKIN)) {
					direction = BlockFace.WEST;
				} else if ((b.getRelative(BlockFace.NORTH_EAST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.SOUTH_EAST, 1).getType() == Material.PUMPKIN)) {
					direction = BlockFace.EAST;
				}
				cannonType = 1;
				cannonLength = 2;
			...
			if ((b.getRelative(BlockFace.EAST, 1).getType() == Material.GOLD_BLOCK) && (b.getRelative(BlockFace.WEST, 1).getType() == Material.GOLD_BLOCK)) {
				if ((b.getRelative(BlockFace.NORTH_WEST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.NORTH_EAST, 1).getType() == Material.PUMPKIN)) {
					direction = BlockFace.NORTH;
				} else if ((b.getRelative(BlockFace.SOUTH_WEST, 1).getType() == Material.PUMPKIN) && (b.getRelative(BlockFace.SOUTH_EAST, 1).getType() == Material.PUMPKIN)) {
					direction = BlockFace.SOUTH;
				}
				cannonType = 1;
				cannonLength = 2;
	...
	public void turnCannonLayer(Boolean right, Player p, int offsetY) {
		...
		for (int x = 0; x < 7; x++) {
			for (int z = 0; z < 7; z++) {
				if ((loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY).getTypeId() == 69) || (loc.getBlock().getRelative(x - 3, 0, z - 3).getRelative(BlockFace.UP, offsetY).getTypeId() == 77)) {
		...
		if (offsetY == 0) {
			...
			Location teleLoc = new Location(p.getWorld(), loc.getBlock().getRelative(direction, -1).getX() + 0.5, loc.getBlock().getRelative(direction, -1).getY(), loc.getBlock().getRelative(direction, -1).getZ() + 0.5);
			...
			if ((cannonTurnCounter < 4) && ((loc.getBlock().getRelative(direction, 1).getRelative(BlockFace.DOWN, 1).getTypeId() == 5) || ((cannonType == 6) && (loc.getBlock().getRelative(direction, 1).getRelative(BlockFace.DOWN, 2).getTypeId() == 5)))) {
				cannonTurnCounter++;
				turnCannon(right, p);
			} else {
				cannonTurnCounter = 0;
	...
	public boolean checkProtectedRegion(Player player, Location loc) {
		if (wgp != null) {
			...
			ApplicableRegionSet set = regionManager.getApplicableRegions(BukkitUtil.toVector(loc));
		...
		nc.getServer().getScheduler().scheduleSyncDelayedTask(nc, new Runnable() {
			...
			public void run() {
				...
				if ((i >= (dc.setDepth + freeFallHt)) || ((dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 8) && (dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 9) && (dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 0) && (dc.warhead.getRelative(BlockFace.DOWN, 2).getTypeId() != 79))) {
					...
					/*
					 * int fuseDelay = 5; for (int x = 0; x < 9; x++) { for( int y = 0; y < 9; y++ )
					 * { for (int z = 0; z < 9; z++) { double ranNum = Math.random(); int xdist =
					 * Math.abs(x - 4); int ydist = Math.abs(y - 4); int zdist = Math.abs(z - 4);
					 * int dist = xdist+ydist+zdist; Block theBlock = currentBlock.getRelative(x -
					 * 4, y - 4, z - 4); int blockType = theBlock.getTypeId();
					 *
					 * double breakChance = 1; if( Craft.blockHardness(blockType) == 3 ) {
					 * breakChance = 0;
					 *
					 * }else if( Craft.blockHardness(blockType) == 46 ) { breakChance = -1;
					 *
					 * }else if( dist > 9 ) { if( Craft.blockHardness(blockType) == 2 ) {
					 * breakChance = .1; }else if( Craft.blockHardness(blockType) == 1 ) {
					 * breakChance = .3; } }else if( dist > 6 ) { if( Craft.blockHardness(blockType)
					 * == 2 ) { breakChance = .3; }else if( Craft.blockHardness(blockType) == 1 ) {
					 * breakChance = .6; } }else if( dist > 3 ) { if( Craft.blockHardness(blockType)
					 * == 2 ) { breakChance = .6; }else if( Craft.blockHardness(blockType) == 1 ) {
					 * breakChance = .9; } }
					 *
					 * if( breakChance == -1 ) {
					 *
					 * TNTPrimed tnt = (TNTPrimed)theBlock.getWorld().spawnEntity(new
					 * Location(theBlock.getWorld(), theBlock.getX(), theBlock.getY(),
					 * theBlock.getZ()), EntityType.PRIMED_TNT); tnt.setFuseTicks(fuseDelay);
					 * fuseDelay = fuseDelay + 2; }else if( ranNum < breakChance ) { if(
					 * theBlock.getY() >= 63 ) theBlock.setType(Material.AIR); else
