	...
	public static boolean detect(Craft craft, int X, int Y, int Z, boolean autoShip){
		...
		//the recursive algorithm returned before the max block count have been reached, we have a craft !
           ...
           offZ = (float)(player.getZ() - posZ);
				...
				//	craft.captain.sendMessage(ChatColor.YELLOW +  craft.type.sayOnControl);
				//else
				//	craft.captain.sendMessage(ChatColor.YELLOW + "You launch the " + ChatColor.WHITE + craft.customName + ChatColor.YELLOW + " !");
