...
*/
	...
	public short getWorldBlockId(int x, int y, int z, int r, int minX, int minY, int minZ, int offX, int offZ){
		...
		/*blockId = (short) craft.world.getBlockTypeIdAt(minX + rotateX(x - offX, z - offZ, r),
				minY + y,
				minZ + rotateZ(x - offX, z - offZ, r));*/
				...
				for(int y=0;y<newSize.getBlockY();y++){
					...
					if(newMatrix[x][y][z]!=255){ //before move : craft block
						...
						if(getCraftBlockId(x + dx + newminX, y + dy, z + dz + newminZ, dr) == 255){
							...
							if(!canGoThrough(getWorldBlockId(x + dx, y + dy, z + dz, newRotation, newminX, craft.minY, newminZ, newoffsetX, newoffsetZ))){
								//craft.player.sendMessage("failed at " + x + "," + y + "," + z );
								//craft.player.sendMessage("newMinX=" + newminX + " newMinZ=" + newminZ + " newoffsetX=" + newoffsetX + " newoffsetZ=" + newoffsetZ);
								//craft.player.sendMessage("newSize.getBlockX()= " + newSize.getBlockX() + " newSize.getBlockZ()=" + newSize.getBlockZ() );
								//craft.player.sendMessage("minX=" + craft.minX + " minZ=" + craft.minZ + " offX=" + craft.offX + " offZ=" + craft.offZ);
								//craft.player.sendMessage("craft.sizeX= " + craft.sizeX + " craft.sizeX=" + craft.sizeZ );
								return false;
							}
						}
					}
	...
	public void turn(int dr) {
		...
		/*if( craft.helmDestroyed )
		{
			if( craft.driverName != null )
			{
				Player p = plugin.getServer().getPlayer(craft.driverName);
				if( p != null )
					p.sendMessage("Helm Control Destroyed!");
			}
			return;
		}*/
		...
		for (Entity e : craftEntities) {
			...
			//double newoffsetX = rotateX( (double)(oldLoc.getX() -(craft.minX+craft.offX)), (double)(oldLoc.getZ() - (craft.minZ+craft.offZ)), dr);
			//double newoffsetZ = rotateZ( (double)(oldLoc.getX() -(craft.minX+craft.offX)), (double)(oldLoc.getZ() - (craft.minZ+craft.offZ)), dr);
		...
		//cm = new CraftMover(craft, plugin);
		//cm.structureUpdate(null,false);
	}
