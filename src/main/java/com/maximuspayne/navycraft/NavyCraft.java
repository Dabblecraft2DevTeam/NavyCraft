...
/**
 * MoveCraft plugin for Hey0 mod (hMod) by Yogoda Ported to Bukkit by SycoPrime
 * Modified by Maximuspayne for NBZ Continuing to be modified by Solmex, for
 * public use Licensed under Apache 2.0
 *
 * You are free to modify it for your own server or use part of the code for
 * your own plugins. You don't need to credit me if you do, but I would
 * appreciate it :)
 *
...
public class NavyCraft extends JavaPlugin {
	...
	public static enum battleTypes {
		battle1, battle2
	}; // 1
	...
	public boolean DebugMessage(String message, int messageLevel) {
		/*
		 * Message Levels: 0: Error 1: Something I'm currently testing 2: Something I
		 * think I just fixed 3: Something I'm pretty sure is fixed 4: Supporting
		 * information 5: Nearly frivolous information
		 */
	...
	public Craft createCraft(Player player, CraftType craftType, int x, int y, int z, String name, float dr, Block signBlock) {
		...
		if (DebugMode == true) {
			player.sendMessage("Attempting to create " + craftType.name + "at coordinates " + Integer.toString(x) + ", " + Integer.toString(y) + ", " + Integer.toString(z));
		}
	...
	public void structureUpdateScheduler() {
		...
		getServer().getScheduler().scheduleSyncRepeatingTask(this, () -> {
			...
			int vehicleCount1 = Craft.craftList.size();
			int vehicleNum1 = (schedulerCounter) % vehicleCount1;
			int updateNum1 = (schedulerCounter / vehicleCount1) % 4;

			try {
				if (vehicleCount1 < 10) {
					updateCraft(vehicleNum1, updateNum1);
					schedulerCounter++;
				} else if ((vehicleCount1 >= 10) && (vehicleCount1 < 20)) {
					// vehicleNum = (vehicleNum + vehicleCount/2)%vehicleCount;
					updateCraft(vehicleNum1, updateNum1);
					vehicleNum1 = (vehicleNum1 + 1) % vehicleCount1;
				...
				} else if ((vehicleCount1 >= 20) && (vehicleCount1 < 30)) {
				...
				if (checkCraft.isMoving) {
					...
					if (updateNum == 0) {
						if (checkCraft.doRemove) {
							...
							checkCraft.destroy();
						} else if (((Math.abs(checkCraft.gear) == 1) && ((((schedulerCounter / 4) / vehicleCount) % 3) == 0)) || ((Math.abs(checkCraft.gear) == 2) && ((((schedulerCounter / 4) / vehicleCount) % 2) == 0)) || (Math.abs(checkCraft.gear) == 3)) {
							if (((Math.abs(checkCraft.gear) == 1) && (((System.currentTimeMillis() - checkCraft.lastMove) / 1000) > 8)) || ((Math.abs(checkCraft.gear) == 2) && (((System.currentTimeMillis() - checkCraft.lastMove) / 1000) >= 5)) || ((Math.abs(checkCraft.gear) == 3) && (((System.currentTimeMillis() - checkCraft.lastMove) / 1000.0f) >= 2.5))) {
					...
					for (int k = -r; k <= r; k++) {
						...
						if (Math.abs(i) == r)// if on x edges
						{
							...
							} else if (Math.abs(k) == r) // if on xz edges
							{
								refK = (int) ((Math.abs(k) - 1) * Math.signum(k));
								refPowerMult = 0.40f;
								if ((Math.abs(j) >= (r - 2)) && (Math.abs(j) > 2)) {
									refPowerMult = 0.25f;
								}
							} else if (Math.abs(j) == (r - 1))// if near xy edge
							{
								refPowerMult = 0.40f;
								if ((Math.abs(k) >= (r - 2)) && (Math.abs(k) > 2)) {
									refPowerMult = 0.25f;
								}
						...
						} else if (Math.abs(j) == r)// if on y sides
						{
							...
							if (Math.abs(k) == r) // if on yz edge
							{
								refK = (int) ((Math.abs(k) - 1) * Math.signum(k));
								refPowerMult = 0.40f;
								if ((Math.abs(i) >= (r - 2)) && (Math.abs(i) > 2)) {
									refPowerMult = 0.25f;
								}
